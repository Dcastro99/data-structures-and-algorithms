...
const count = (target, input) => {
  let numTotal = 0;
  for (let arrays of input) {
    let arrTotal = arrays.reduce(
      (addNum, num) => (num === target ? addNum + 1 : addNum),
      0
    );
    numTotal += arrTotal;
  }
...
const totalSum = (input) => {
  ...
  for (let array of input) {
    let arrayTotal = 0;
    array.map((nums) => (arrayTotal += nums));
    numTotal += arrayTotal;
  }
  return numTotal;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let nums = input.map((arr) => {
    // console.log("before", arr);
    arr = arr.filter((num) => typeof num === "number" && num % 5 === 0);
...
let findMaleAndFemale = (data) => {
  ...
  //   .reduce((acc, curr) => (acc += curr.name + " and "), "");
  // console.log(starNames);
  // return starNames.slice(0, -5);
  /////another way
  // let starNames = [];
  // for (let i = 0; i < data.length; i++) {
  //   const obj = data[i];
  //   if (obj.gender === "male" || obj.gender === "female") {
  //     starNames.push(obj.name);
  //   }
  // }
  // return starNames.join(" and ");
  //// second way
  let starNames = data
    .filter((obj) => obj.gender === "male" || obj.gender === "female")
    .map((obj) => obj.name)
