...
const screenForNames = (arr) => {
  const pattern = /(Mr|Ms|Dr|s).\s[A-Za-z]+/gm;
  return arr.filter((string) => pattern.test(string));
};
...
let biggerThanLuke = (arr) => {
  let heavy = arr
    .filter(
      (person) => person.name !== "Luke Skywalker" && Number(person.mass) > 77
    )
...
const detectTicTacToeWin = (board) => {
  ...
  if (helpCheck(0, 0, 0, 1, 0, 2)) return true; //top row check
  ...
  if (helpCheck(0, 1, 1, 1, 2, 1)) return true; //middle col

  if (helpCheck(0, 2, 1, 2, 2, 2)) return true; //right col

  if (helpCheck(0, 2, 1, 1, 2, 0)) return true; //top right diagnal

  if (helpCheck(0, 0, 1, 1, 2, 2)) return true; //top left diagnal
  return false;
};
